...
uninstall_presto() {
    ...
    # macOS: ~/Library/Application Support/ tempo-wallet(config and data share the same dir)
    # Linux: ~/.config/ tempo-wallet(config), ~/.local/share/ tempo-wallet(data)
    ...
    echo ""
    echo " tempo-wallethas been uninstalled."
}

install_local() {
    echo ""
    echo "Building  tempo-walletfrom source..."
