...
```
crates/tempo-wallet/
  src/main.rs            — entry point; parse CLI, run, handle errors
...
| Path | Purpose |
|------|---------|
| `crates/tempo-wallet/src/main.rs` | CLI entry point, module declarations |
| `crates/tempo-wallet/src/cli/run.rs` | Application lifecycle: init, build `Context`, dispatch commands, track analytics |
| `crates/tempo-wallet/src/cli/context.rs` | `Context` struct: shared app state threaded to all commands |
| `crates/tempo-wallet/src/cli/args.rs` | Clap definitions (`Cli`, `QueryArgs`, `Commands`) |
| `crates/tempo-wallet/src/cli/output.rs` | `OutputFormat`, `OutputOptions` |
| `crates/tempo-wallet/src/account/` | Wallet account types (balances, spending limits), on-chain queries |
| `crates/tempo-wallet/src/cli/commands/query/` | Primary query flow: HTTP → 402 detection → payment → retry |
| `crates/tempo-wallet/src/cli/commands/login.rs` | Login command and passkey authentication flow |
| `crates/tempo-wallet/src/cli/commands/logout.rs` | Logout command |
| `crates/tempo-wallet/src/cli/commands/whoami.rs` | Whoami command |
| `crates/tempo-wallet/src/cli/commands/keys.rs` | Key listing with balance and spending limit queries |
| `crates/tempo-wallet/src/cli/commands/sessions/` | Session management commands (list/info/close/sync) |
| `crates/tempo-wallet/src/cli/commands/wallets/` | Wallet management (create, renew, list, fund) |
| `crates/tempo-wallet/src/cli/commands/services/` | Service directory listing and detail views |
| `crates/tempo-wallet/src/http/` | `HttpClient` (reqwest wrapper with retry logic), `HttpRequestPlan`, header/body helpers |
| `crates/tempo-wallet/src/keys/` | Key storage (model, I/O), signer resolution, authorization |
| `crates/tempo-wallet/src/payment/charge.rs` | One-shot on-chain charge payment |
| `crates/tempo-wallet/src/payment/session/` | Session-based payment channel (open, voucher, close, store) |
| `crates/tempo-wallet/src/config.rs` | Config file parsing and RPC resolution |
| `crates/tempo-wallet/src/network.rs` | Built-in network definitions (Tempo, Moderato), explorer URLs |
| `crates/tempo-wallet/src/analytics.rs` | Opt-out PostHog telemetry |
| `crates/tempo-wallet/src/error.rs` | `TempoWalletError` enum (thiserror) |
| `crates/tempo-wallet/src/util.rs` | Formatting helpers, terminal hyperlinks |
