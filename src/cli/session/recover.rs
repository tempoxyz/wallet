...
pub async fn recover_session(
    ...
    let Some(rec) = session_store::load_session(&key)? else {
        match output_format {
            ...
            OutputFormat::Text => {
                ...
                println!(
                    "Use 'presto sessions list --orphaned' to view on-chain channels and 'presto sessions close --orphaned' to close them."
                );
            }
        }
        return Ok(());
    ...
    match query_channel_state(config, &rec.channel_id, &rec.network_name).await {
        Ok(Some((_token, _dep, _settled))) => {
            // Read grace period and compute readiness if close has been requested
            let net_info = config.resolve_network(&rec.network_name)?;
