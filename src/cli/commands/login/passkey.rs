...
use crate::analytics::{
    Analytics, CallbackReceivedPayload, Event, NetworkPayload, WalletCreatedPayload,
};
...
pub(super) struct WalletManager {
...
impl WalletManager {
    ...
    pub(super) fn new(network: NetworkId, analytics: Option<Analytics>) -> Self {
        let auth_server_url = std::env::var("TEMPO_AUTH_URL")
            .ok()
            .unwrap_or_else(|| network.auth_url().to_string());
    ...
    pub(super) async fn setup_wallet(&self, keys: &Keystore) -> Result<String, TempoWalletError> {
        ...
        let auth_base_url = self.auth_base_url();
        ...
        let wallet_address = self.save_keys(keys, callback, local_signer)?;
    ...
    fn save_keys(
        ...
        let default_chain_id = self.network.chain_id();
