    ...
    for ch in &orphaned {
        ...
        match close_discovered_channel(ch, config).await {
            ...
            Ok(CloseOutcome::Pending { remaining_secs }) => {
                ...
                if show_output {
                    eprintln!(
                        "  Pending — {} remaining, run ` tempo-walletsession close --closed` to finalize.",
            ...
            match close_channel_by_id(config, target, network, None).await {
                ...
                Ok(CloseOutcome::Pending { remaining_secs }) => {
                    if output_format == OutputFormat::Json {
                        ...
                        println!(
                            "Channel {target}: close requested — {} remaining, run ` tempo-walletsession close --closed` to finalize.",
                            format_duration(remaining_secs)
                        );
                    }
                }
            ...
            match close_session_from_record(&record, config).await {
                ...
                Ok(CloseOutcome::Pending { remaining_secs }) => {
                    if output_format == OutputFormat::Json {
                        ...
                        println!(
                            "Session for {target}: close requested — {} remaining, run ` tempo-walletsession close --closed` to finalize.",
                            format_duration(remaining_secs)
                        );
                    }
                }
