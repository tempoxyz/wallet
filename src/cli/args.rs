...
impl Cli {
    ...
    ///
    /// Allows `presto https://example.com` as shorthand for
    /// `presto query https://example.com`. Returns `None` when
    ...
    fn try_implicit_query(args: &[String]) -> Option<Self> {
        ...
        if let Some(Commands::Query(ref q)) = cli.command {
            ...
            if !url.contains("://") && !url.contains("localhost") && !url.contains('.') {
                eprintln!(
                    "error: '{url}' is not a presto command. \
                     See 'presto --help' for a list of available commands."
                );
