...
impl Directory {
    ...
    async fn fetch() -> Result<Directory> {
        ...
        let response = client
            ...
            .send()
            .await
            .map_err(|e| PgetError::Http(format!("Failed to fetch directory: {}", e)))?;

        if !response.status().is_success() {
            return Err(PgetError::Http(format!(
        ...
        let directory: Directory = response
            .json()
            .await
            .map_err(|e| PgetError::Http(format!("Failed to parse directory: {}", e)))?;
