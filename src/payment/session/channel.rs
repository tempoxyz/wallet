...
pub async fn find_all_channels_for_payer(
    ...
    for network in &networks {
        ...
        let network = *network;
        network_futures.push(async move {
            scan_network_channels(network, rpc_url, event_topic, payer_topic).await
        });
    }

    let network_results = futures::future::join_all(network_futures).await;
    network_results.into_iter().flatten().collect()
}

/// Scan a single network for open channels where `payer` is the sender.
async fn scan_network_channels(
    network: Network,
    rpc_url: url::Url,
    event_topic: B256,
    payer_topic: B256,
    ...
    while chunk_end > earliest {
        ...
        for log in &logs {
            ...
            let on_chain = match get_channel_on_chain(&provider, escrow, channel_id, log_salt).await
            {
                Ok(Some(ch)) => ch,
                Ok(None) => continue,
                Err(e) => {
