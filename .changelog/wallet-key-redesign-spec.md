...
### ` tempo-walletwallet create [--name NAME]`
...
### ` tempo-walletwallet create [--name NAME] --passkey`

Create a passkey-based wallet via browser auth flow (current ` tempo-walletlogin` behavior).
...
To switch wallets, run ' tempo-walletlogout' first.
```

### ` tempo-walletwallet delete <name> [--yes]`
...
### ` tempo-walletwallet delete --passkey [--yes]`

Remove the passkey wallet. Finds the key entry with `wallet_type = "passkey"` and removes it. Same auto-switch behavior as above.

### ` tempo-walletlogin`

Alias for ` tempo-walletwallet create --passkey`.

### ` tempo-walletlogout [--yes]`

Alias for ` tempo-walletwallet delete --passkey`.
...
- ` tempo-walletwallet create --passkey`: rejects if a `wallet_type = "passkey"` entry already exists.
- ` tempo-walletwallet create`: rejects if the target `--name` already exists (use a different name).
- Both: do not enforce mutual exclusion between local and passkey — both can coexist.

### ` tempo-walletlogout` only affects passkey wallets

` tempo-walletlogout` finds and removes the `wallet_type = "passkey"` entry. It does not touch local wallets. Use ` tempo-walletwallet delete <name>` for local wallets.
...
- ` tempo-walletwhoami` / ` tempo-walletbalance`
- ` tempo-walletsession` commands
- ` tempo-walletcompletions`
...
- ` tempo-walletwallet list` — show all wallets if multiple exist.
