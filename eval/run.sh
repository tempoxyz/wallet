...
# Check installed  tempo-walletmatches the repo version
if command -v  tempo-wallet&>/dev/null; then
  ...
  if [ -n "$BUILD_HASH" ] && [ "$INSTALLED_HASH" != "$BUILD_HASH" ]; then
    echo "Warning: installed  tempo-wallet($(which presto)) differs from repo build"
