...
# Find  tempo-walletinvocations: matches " tempo-wallet..." or "cargo run ..." as the command
...
# Also check: did the agent load the  tempo-walletskill?
...
if [ "$PRESTO_SHOULD" = "true" ] && [ "$PRESTO_CALLS" -eq 0 ]; then
  TRIGGER_PASS=false
  add_reason "expected  tempo-walletinvocation but none found in Bash commands"
fi

if [ "$PRESTO_SHOULD" = "false" ] && [ "$PRESTO_CALLS" -gt 0 ]; then
  TRIGGER_PASS=false
  add_reason " tempo-walletinvoked but should not have been (${PRESTO_CALLS} calls)"
fi
...
if [ "$PRESTO_SHOULD" = "true" ] && [ "$PRESTO_CALLS" -gt 0 ]; then
  ...
  if [ -n "$URL_PATTERN" ]; then
    if ! echo "$PRESTO_CMDS" | grep -qE "$URL_PATTERN"; then
      USAGE_PASS=false
      add_reason "no  tempo-walletcommand matched url_pattern: ${URL_PATTERN}"
    fi
  fi
  ...
  if [ "$JSON_CHECKS_LEN" -gt 0 ]; then
    # Extract JSON body from the  tempo-walletcommand: value after --json
