...
fn keys_list_json_and_toon_have_expected_shape() {
    ...
    assert!(toon.get("keys").is_some());
    assert!(toon.get("total").is_some());
}

#[test]
fn wallets_list_empty_json_and_toon_shape() {
    let temp = TestConfigBuilder::new().build();
    let (json_out, json, toon_out, toon) = run_both(&temp, &["wallets", "list"]);
    assert_clean_stderr(&json_out);
    assert_clean_stderr(&toon_out);
    assert!(json.get("wallets").is_some());
    assert!(json.get("total").is_some());
    assert!(toon.get("wallets").is_some());
    assert!(toon.get("total").is_some());
}

#[test]
fn logout_without_wallet_json_and_toon_shape() {
    let temp = TestConfigBuilder::new().build();
...
#[test]
fn keys_clean_missing_file_json_and_toon_shape() {
    let temp = TestConfigBuilder::new().build();
    let (json_out, json, toon_out, toon) = run_both(&temp, &["keys", "clean", "--yes"]);
    assert_clean_stderr(&json_out);
    assert_clean_stderr(&toon_out);
    assert_eq!(json["cleaned"], false);
    assert!(json.get("path").is_some());
    assert_eq!(toon["cleaned"], false);
    assert!(toon.get("path").is_some());
}
