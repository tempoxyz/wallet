...
impl Keystore {
    ...
    pub(crate) fn primary_key(&self) -> Option<&KeyEntry> {
    ...
    pub(crate) fn wallet_address(&self) -> &str {
        self.primary_key()
    ...
    pub(crate) fn is_provisioned(&self, network: NetworkId) -> bool {
    ...
    pub(crate) fn key_for_network(&self, network: NetworkId) -> Option<&KeyEntry> {
    ...
    pub(crate) fn key_for_wallet_and_network(
        &self,
        wallet_address: &str,
        network: NetworkId,
    ) -> Option<&KeyEntry> {
    ...
    pub(crate) fn find_passkey_wallet(&self) -> Option<&KeyEntry> {
        self.keys
            .iter()
            .find(|k| k.wallet_type == WalletType::Passkey)
    }
    ...
    pub(crate) fn upsert_by_wallet_and_chain(
        &mut self,
        wallet_address: &str,
        chain_id: u64,
    ) -> &mut KeyEntry {
