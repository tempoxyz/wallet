...
impl Analytics {
    pub(crate) async fn new(network: NetworkId, config: &Config, keys: &Keystore) -> Option<Self> {
    ...
    pub(crate) fn identify(&self, keys: &Keystore) {
        let addr = keys.wallet_address();
        if addr.is_empty() {
            return;
        }
...
pub(crate) trait EventPayload: Serialize + Send + Sync + 'static {}
