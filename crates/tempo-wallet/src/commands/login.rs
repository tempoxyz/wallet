...
/// OWS login — headless alternative to passkey login.
///
/// Same on-chain result: root wallet + scoped access key ($100, 30 days).
/// Root key lives in OWS vault instead of passkey secure enclave.
async fn run_ows(ctx: &Context) -> Result<(), TempoError> {
    ...
    // Create root wallet in OWS vault (encrypted, never on disk as plaintext).
    let mut nonce = [0u8; 4];
    ...
    // Derive wallet address from root key (briefly in-memory, wiped on drop).
    ...
    // Generate scoped access key — same defaults as passkey login.
    ...
    // Save access key + authorization. Root key stays in OWS vault.
    let access_key_hex = zeroize::Zeroizing::new(
        format!("0x{}", hex::encode(access_signer.to_bytes())),
    );
    let mut keys = ctx.keys.clone();
    let entry = keys.upsert_by_wallet_address_and_chain(wallet_address, ctx.network.chain_id());
    entry.wallet_type = tempo_common::keys::WalletType::Local;
    entry.set_wallet_address(wallet_address);
    entry.set_key_address(Some(access_signer.address()));
